...
function init_env!(
    ...
    #-----------------------------------#
    # find instruments in the columns   #
    #-----------------------------------#
    ask_col = instr * "_" * "o_ask_px_1"
    bid_col = instr * "_" * "o_bid_px_1"
    @assert (ask_col in names_) && 
            (bid_col in names_) && 
            ("ts"  in names_)
...
Env(df::DataFrame,
    ...
    end_idx::Int = -1,
    need_to_scale::Bool = true,
    ) = init_env!(df, instrs, w_size, commission, lat_ms, exclude_cols, scaler, start_idx, end_idx, need_to_scale)
...
function execute!(env::Env, step::Int=2, realize_gain::Bool = true)
    ...
    MinA = min(env.data[env.last_point[] : env.last_point[] + env.w_size, env.ask_col]...)
    MaxB = max(env.data[env.last_point[] : env.last_point[] + env.w_size, env.bid_col]...)
